...
class SimpleTimeSeriesForecastingRunner(BaseTimeSeriesForecastingRunner):
    ...
    def forward(self, data: tuple, epoch: int = None, iter_num: int = None, train: bool = True, **kwargs) -> tuple:
        ...
        if train:
            ...
            future_data_4_dec = self.select_input_features(future_data)
            # 仅在非训练模式下清空时间特征的第一个通道（假设是时间步特征）
            # 这行代码的含义是，在测试或验证时，通常我们会给解码器一个空的未来时间步输入，
            # 只有模型内部的时序编码器会使用真实的未来时间信息。
            # 如果你的模型设计不是这样，可以移除或调整。
            future_data_4_dec[..., 0] = torch.empty_like(future_data_4_dec[..., 0])

        # curriculum learning
        # 从模型获取主任务预测和 SSL 相关数据
        main_prediction_data, ssl_data = self.model(history_data=history_data, future_data=future_data_4_dec,
        ...
        return prediction, real_value, ssl_data
